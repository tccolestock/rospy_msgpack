    ...
    def multi_dof_joint_trajectory(cls, obj):
        ...
        for i in range(msg['_length_points']):
            ...
            for j in range(msg['%s_length_trans' %i]):
                tr = encode.translation(obj.points[i].transfroms[j].translation, "%s_%s" %(i,j))
                msg.update(tr)
                rot = encode.rotation(obj.points[i].transforms[j].rotation, "%s_%s" %(i,j))
                msg.update(rot)
            msg['%s_length_velo' %i] = len(obj.points[i].velocities)
            for j in range(msg['%s_length_velo' %i]):
                vl = encode.linear(obj.points[i].velocities[j].linear, "%s_%s_v" %(i,j))
                msg.update(vl)
                va = encode.angular(obj.points[i].velocities[j].angular, "%s_%s_v" %(i,j))
                msg.update(va)
            msg['%s_length_acc' %i] = len(obj.points[i].accelerations)
            for j in range(msg['%s_length_acc' %i]):
                al = encode.linear(obj.points[i].accelerations[j].linear, "%s_%s_a" %(i,j))
                msg.update(al)
                aa = encode.angular(obj.points[i].accelerations[j].angular, "%s_%s_a" %(i,j))
                msg.update(aa)
...
class Decode():
    ...
    def multi_dof_joint_trajectory(cls, msg, obj):
        ...
        for i in range(msg['_length_points']):
            ...
            for j in range(msg['%s_length_trans' %i]):
                tr = Transform()
                tr.translation = decode.translation(msg, tr.translation, "%s_%s" %(i,j))
                tr.rotation = decode.rotation(msg, tr.rotation, "%s_%s" %(i,j))
